...
\BlankLine
\tcp{Step 1: Set Maximum Transmission Power}
\For{each interface in $\boldsymbol{G}$}{
    $P_i \gets$ Max Tx power for given interface\;
    groupState[interface].TxDiff $\gets (P_i - interface.TxPower)$\;
}
\BlankLine
...
\BlankLine
\tcp{Step 3: Adjust Transmission Power (if enabled)}
\Repeat{no improvement in groupInterference}{
    \tcp{Identify the "worst interface" $interface_{worst}$ --- the one causing the most interference for other APs}
    $interface_{worst}$ $\gets \argmax_{w_i \in G} FromIfaceInterference(groupState, w_i)$\;
    newTxPower $\gets \frac{1}{2}(interface_{worst}.TxPower + minTxPower)$ \;
    groupState[$interface_{worst}$].TxPower $\gets$ newTxPower\;
    groupInterference $\gets$ GroupInterference(scandata)\;
}
\BlankLine
\tcp{Step 4: Send configuration changes to AP}
...
Function \textit{OnIfaceInterference(groupState, interface, channel)} calculates interference experienced by given access point (and thus by its wireless interface) from outer and inner signals.
...
In \textit{ChannelInterference()}, it is assumed that the distance between orthogonal channels is 5 MHz. Thus, given a 2.4 GHz band and 20 MHz width, the distance between two channels should be at least 5. The interval of length $n$ should have $n-1$ channels in between, which is the reason for the $add$ factor.
...
If $w_i$ interferes with currently considered interface $w, w \ne w_i$, all measurements of $w$ on $w_i$ are computed as a cumulative factor, and maximum RSSI of $w$ on some other interface $w_j$ is saved. Then, the \textit{worst interface}, i.e., the one causing the most interference on others, is chosen to reduce its transmit power nearly  in two, to the nearest defined power value. Then group interference is recalculated in an iterative way while there is an improvement.
...
\begin{itemize}
    ...
    % \item It is avoided, for example by hostapd automatic channel selection, to rely on pre-defined signal strength levels, since RSSI estimation can differ across different wireless cards. Instead, a minim
    \item Noise floor should be included into the calculations, since it can drastically affect the choice of a channel;
    % \item Two-stage adjustment, first channel, then transmit power, is arguably suboptimal. The 
